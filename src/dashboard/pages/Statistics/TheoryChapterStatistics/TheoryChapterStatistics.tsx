...
import {clearStatisticsUserDetail, getStatisticsUserDetailTheory} from "../../../../redux/reducers/coursesReducer"
import {getStatisticsUserDetailTheorySelector} from "../../../../redux/selectors/coursesSelectors";
...
const TheoryChapterStatistics = () => {
    ...
    const theory = useAppSelector(state => getStatisticsUserDetailTheorySelector(state))
    ...
    useEffect(() => {
        ...
        dispatch(getStatisticsUserDetailTheory(req))
        return () => {
    ...
    if (!theory?.intro && !theory?.chapter) {
        return <Preloader/>
    }

    return (
        <div className='createLesson'>
            <h2 className='title'>Статистика главы '{theory.title}'</h2>
            {/*// @ts-ignore*/}
            <Table columns={columns} pagination={false} scroll={{y: 540}}
                   dataSource={theory?.chapter || theory?.intro}/>
        </div>
    )
}
export default TheoryChapterStatistics